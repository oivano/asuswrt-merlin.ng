...
export RT-AC68U_BASE := IPV6SUPP=y HTTPS=y ARM=y AUTODICT=y BBEXTRAS=y USBEXTRAS=y EBTABLES=y \
			...
			NANO=y OPENSSL11=y NTPD=y DNSPRIVACY=y NETOOL=y TRACEROUTE=y PPTP=n L2TP=n CLOUDSYNC=n \
			YANDEXDNS=n FTP=n MERLINUPDATE=n WIREGUARD=y CAPTCHA=y #NFCM=n
...
export RT-AC68U += BUILD_NAME="RT-AC68U" NEWSSID_REV2=y UUPLUGIN=y AVBLCHAN=y ASUSCTRL=y \
		   AHS=n ASD=y FRS_LIVE_UPDATE=n OPEN_NAT=n SW_CTRL_ALLLED=y NVRAM_ENCRYPT=y
export RT-AC68U += JFFS_NVRAM=y

DSL-AC68U-NONE := LOGO_LED=y LED_BTN=y ROG=y MEDIASRV=y EMAIL=y DWB=y ETHOBD=y PRINTER=y ACCEL_PPTPD=y SMARTSYNCBASE=y CONNDIAG=y \
		  NATNL_AICLOUD=y NATNL_AIHOME=y OPTIMIZE_XBOX=y GAME_MODE=y TIMEMACHINE=y FORCE_ROAMING=y BCN_RPT=y BTM_11V=y \
		  HSPOT=y SNMPD=y MULTICASTIPTV=y OOKLA=y FILEFLEX=y WIREGUARD=y CAPTCHA=y REPEATER=y DNSFILTER=y NANO=y DNSPRIVACY=y NEW_USER_LOW_RSSI=y AMAS=y
export DSL-AC68U := $(filter-out $(DSL-AC68U-NONE),$(RT-AC68U_BASE))
export DSL-AC68U += BUILD_NAME="DSL-AC68U" TFAT=y NTFS="tuxera" HFS="tuxera" LLDPD_1_0_11=y CFGSYNC=y LIBASUSLOG=y DSL=y DSL_TCLINUX=y VDSL=y BTN_WPS_ALLLED=y TCPDUMP=y BCM4352_5G=y JFFS_NVRAM=y AHS=y ASD=n FRS_LIVE_UPDATE=y
